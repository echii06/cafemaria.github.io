...
if(count($_POST)>0 && $captcha == true) {
    $result = $mysqli->query("SELECT * FROM staff WHERE userName='" . $_POST["userName"] . "' and password = '". $_POST["password"]."'");
    $row  = $result->fetch_assoc();
    $result->free();
    if(is_array($row)) {
...
<?php } ?>
                ...
                <input name="captcha_code" type="text"><br><br><img src="captcha_code.php" />
                <?php } ?>
